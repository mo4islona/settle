...
fn open_with_fn_reducer() -> SettleStream {
    let mut db = SettleStream::open(Config::new(EXTERNAL_PNL_SCHEMA)).unwrap();
...
fn external_reducer_matches_event_rules() {
    ...
    let mut er_db = SettleStream::open(Config::new(DEX_SCHEMA)).unwrap();
...
fn external_reducer_crash_recovery() {
    ...
    // Phase 1: Process blocks with external reducer, finalize block 1000
    {
        ...
        let mut db = SettleStream::open(config).unwrap();
        db.set_reducer_runtime("pnl", Box::new(pnl_fn_runtime()))
            .unwrap();
