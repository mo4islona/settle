...
fn open_with_valid_schema() {
    let db = SettleStream::open(Config::new(SIMPLE_SCHEMA));
    assert!(db.is_ok());
}

#[test]
fn open_with_invalid_schema() {
    let db = SettleStream::open(Config::new("INVALID SQL GARBAGE"));
    assert!(db.is_err());
}

#[test]
fn simple_ingest_and_flush() {
    let mut db = SettleStream::open(Config::new(SIMPLE_SCHEMA)).unwrap();
...
fn backpressure_signal() {
    let mut db = SettleStream::open(Config::new(SIMPLE_SCHEMA).max_buffer_size(3)).unwrap();
...
fn ingest_persists_and_restores_state() {
    ...
    {
        let mut db =
            SettleStream::open(Config::with_data_dir(schema, dir.path().to_str().unwrap())).unwrap();
