//! Uniswap V2/V3 Swap Analytics — Complex SettleStream Example
//!
...
use settle_stream::types::{ChangeBatch, ChangeRecord, RowMap, Value};
...
fn schema_parses_successfully() {
    SettleStream::open(Config::new(UNISWAP_SCHEMA)).unwrap();
}

/// Direct stablecoin pricing: WETH/USDC swap produces correct USD price.
#[test]
fn direct_stablecoin_pricing() {
    let mut db = SettleStream::open(Config::new(UNISWAP_SCHEMA)).unwrap();
