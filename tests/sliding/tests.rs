...
fn sliding_window_basic_volume_tracking() {
    ...
    assert_eq!(eth_insert.values.get("trade_count"), Some(&Value::UInt64(1)));
}
...
fn sliding_window_expiry_reduces_volume() {
    ...
    db.process_batch("trades", 2, vec![
        make_trade("ETH", 200.0, 2100.0, 1_800_000),
    ]).unwrap();
    db.flush().unwrap();

    // Block 3: ETH trade at t=1hr+1s, volume=50
    // This should expire block 1 (ts=0 < cutoff=1000)
    db.process_batch("trades", 3, vec![
        make_trade("ETH", 50.0, 2200.0, 3_601_000),
    ]).unwrap();
...
fn sliding_window_rollback_and_reprocess() {
    ...
    db.process_batch("trades", 2, vec![
        make_trade("ETH", 200.0, 2100.0, 1_000_000),
    ]).unwrap();
    db.process_batch("trades", 3, vec![
        make_trade("ETH", 300.0, 2200.0, 2_000_000),
    ]).unwrap();
    db.flush().unwrap();
    ...
    db.process_batch("trades", 2, vec![
        make_trade("ETH", 50.0, 1900.0, 1_500_000),
    ]).unwrap();
    let new_batch = db.flush().unwrap();

    let vol_recs = records_for_table(&new_batch, "volume_1h");
    let eth_new = vol_recs.iter().find(|r|
        r.key.get("pair") == Some(&Value::String("ETH".into()))
    ).expect("should have ETH update after re-ingest");
    assert_eq!(eth_new.values.get("total_volume"), Some(&Value::Float64(150.0)));
}
...
fn sliding_window_multiple_groups_and_windows() {
    ...
    let vol_recs = records_for_table(&batch, "volume_1h");
    let eth_vol = vol_recs.iter().find(|r|
        r.key.get("pair") == Some(&Value::String("ETH".into()))
    ).expect("ETH volume_1h");
    assert_eq!(eth_vol.values.get("total_volume"), Some(&Value::Float64(350.0)));

    // stats_30m: block 1 expired for ETH and BTC
    let stats_recs = records_for_table(&batch, "stats_30m");
    let eth_stats = stats_recs.iter().find(|r|
        r.key.get("pair") == Some(&Value::String("ETH".into()))
    ).expect("ETH stats_30m");
    // ETH: blocks 2+3 remain (prices: 2100, 2200; volumes: 200, 50)
    assert_eq!(eth_stats.values.get("vol_sum"), Some(&Value::Float64(250.0)));
    assert_eq!(eth_stats.values.get("price_min"), Some(&Value::Float64(2100.0)));
    assert_eq!(eth_stats.values.get("price_max"), Some(&Value::Float64(2200.0)));
    assert_eq!(eth_stats.values.get("price_first"), Some(&Value::Float64(2100.0)));
    assert_eq!(eth_stats.values.get("price_last"), Some(&Value::Float64(2200.0)));
}
...
fn sliding_window_complete_expiry_deletes_group() {
    ...
    let doge_delete = vol_recs.iter().find(|r|
...
fn sliding_window_with_finalization() {
    ...
    for i in 0..5u64 {
        db.process_batch("trades", i + 1, vec![
            make_trade("ETH", (i + 1) as f64 * 10.0, 2000.0, (i * 600_000) as i64),
        ]).unwrap();
    }
    ...
    // Block 6 at t=1hr+1s
    db.process_batch("trades", 6, vec![
        make_trade("ETH", 100.0, 2500.0, 3_601_000),
    ]).unwrap();
    let batch = db.flush().unwrap();
