...
| `Uint256` | 256-bit unsigned integer for token amounts and balances. Stored as big-endian bytes; use `Float64` for approximate arithmetic. |
| `String` | UTF-8 string |
| `DateTime` | Timestamp (milliseconds) |
| `Boolean` | Boolean |
| `Bytes` | Arbitrary byte array |
| `Base58` | Base58-encoded byte string. Used for Solana addresses, Bitcoin addresses, IPFS CIDs, and other identifiers that use Base58 or Base58Check encoding. |
