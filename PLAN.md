# SettleStream — Implementation Plan

Based on [RFC_SETTLE_STREAM.md](./RFC_SETTLE_STREAM.md). Targets **Phase 1 (PoC)** scope.
...
- [x] `SettleStreamEngine`: topological sort, pipeline processing (raw → reducer → MV)
...
- [x] `SettleStream` struct: open, process_batch, rollback, finalize, flush, ack
...
- [x] napi-rs bindings: `JsSettleStream` class with open, processBatch, rollback, finalize, flush, ack
- [x] TypeScript type definitions (index.d.ts, pipes.d.ts)
- [x] `settleStreamTarget` Pipes SDK target (pipes.js): write loop, fork handler, finalization
...
- [x] **Pipeline uses RowMaps** — `ReducerRuntime`, `ReducerEngine`, `MVEngine`, and `SettleStreamEngine` all operate on `RowMap` (HashMap). No RowMap↔Row conversions in the hot path.
- [x] **Public API** — `SettleStream::process_batch` accepts `Vec<RowMap>`. RowMaps flow through the pipeline unchanged.
...
- [x] **SettleStreamEngine::finalize()** — passes batch through to each reducer.
- [x] **SettleStream::finalize() and ingest()** — create batch, collect reducer state + metadata, call `storage.commit(&batch)` once.
- [x] **Bug fix** — `SettleStream::finalize()` previously did not persist metadata (only `ingest()` did). Now both paths commit metadata atomically.
